...
mod_check <- function(param.weed=param.weed, area=area, duration=duration, repetitions=repetitions, crop_list=crop_list, max_vec_length=max_vec_length)
#if(anyNA(af)){af <- NULL}
#n_loci <- length(af)
...
prob_surv_summer <- quanti(origin= "germ_dorm", step_name="Pindeath", crop=crop, area=area, res_max=1, res_min=0)
pop_step(start="germ_dorm", result="SB_summer", surv_prob=prob_surv_summer)
